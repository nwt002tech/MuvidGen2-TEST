...
def render():
    ...
    with st.sidebar:
        ...
        st.caption("✅ HF token present" if token else "ℹ️ No HF token — using fallback images.")
        st.session_state["force_fallback"] = st.toggle("Force fallback (skip HF calls)", value=False)
        st.session_state["model_id"] = st.text_input("HF Image Model ID", value="stabilityai/sdxl-turbo")
        st.subheader("Visual Style")
        visual_style = st.text_area("Base style", value="Pixar-inspired 3D, kid-friendly, soft studio lighting, GI, SSS, PBR, cinematic DOF", height=80)
        ...
        camera = st.text_input("Camera Direction", value="gentle dolly, slow zoom-in; 35mm lens eq.")
        model_hint = st.text_input("Model Hint", value="family-safe, cheerful, consistent style")
        character_bible = st.text_area("Character Bible", value="Green letter F, friendly face, classroom setting.", height=80)
        negative_hint = st.text_input("Avoid / negatives", value="no text, no watermark, no logo, no scary, no photoreal humans")
    ...
    if render_btn:
        ...
        for i, line in enumerate(lines):
            ...
            if token and not st.session_state.get("force_fallback", False):
                img = hf_txt2img(prompt, token, st.session_state.get("model_id", "stabilityai/sdxl-turbo"))
        ...
        out_path, vid_dur = assemble_video(images, lines, audio_path, beats, total_duration=duration)
